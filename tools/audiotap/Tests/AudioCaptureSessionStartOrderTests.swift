...
final class AudioCaptureSessionStartOrderTests: XCTestCase {
    ...
    private final class FakeMicSession: MicEngineSessionProviding {
        ...
        /// Where the handler writes. Read in `installTap` to pin the ordering the
        /// removal tests depend on.
        private let outputURL: URL
        /// Whether the handler's WAV was already on disk when `installTap` ran.
        private(set) var fileExistedAtInstallTap = false
        ...
        init(log: StartLog, seam: MicSeam, outputURL: URL) {
            self.log = log
            self.seam = seam
        ...
        func installTap(format _: AVAudioFormat, block _: AVAudioNodeTapBlock) throws {
            fileExistedAtInstallTap = FileManager.default.fileExists(atPath: outputURL.path)
    ...
    private func makeFixture(mic: MicSeam = .succeeds, app: AppSeam = .succeeds) -> Fixture {
        ...
        let micSession = FakeMicSession(log: log, seam: mic, outputURL: micURL)
        addTeardownBlock {
        ...
        return Fixture(
            session: session, log: log, micSession: micSession, appURL: appURL, micURL: micURL,
        )
    }
    ...
    func testStartOpensTheMicrophoneBeforeTheAppTap() throws {
        ...
        // The load-bearing half of that control: the handler creates its WAV
        // *before* the call the `failsAfterCreatingItsFile` arm throws from.
        // Reorder `startEngine` so the file comes after `installTap` and that
        // arm would pass with nothing removed, on a file that never existed.
        XCTAssertTrue(
            fixture.micSession.fileExistedAtInstallTap,
            "the WAV must already exist when installTap runs, or the removal tests go vacuous",
        )
