...
public class AudioCaptureSession {
    ...
    ///
    /// Stated to the evidence, because the evidence does not reach as far as it
    /// is tempting to claim. What the order guarantees is that the input open is
    /// *requested* before the aggregate exists, not that anything it sets off has
    /// finished. Which part of it kills the tap is not established. A field log
    /// has the rate change arriving 836 ms after the microphone open, which is
    /// long after a healthy tap's first callback, so either the disturbance is
    /// the open itself (the A2DP stream torn down to begin call-profile
    /// negotiation, stalling the output stream at once) and the rate change is
    /// the survivable tail, or the flip latency is sometimes short enough to land
    /// inside the roughly 50 ms between creating the aggregate and its first
    /// cycle. Both fit the numbers, both are set off ahead of the aggregate by
    /// this order, and neither is closed by it: whatever lands inside that window
    /// anyway is the residual, and 30 clean runs bound it rather than remove it.
    /// Telling them apart needs a run that FAILS, so it is the old-order control
    /// group that answers it, not this one: compare the timestamps of `Mic
    /// recording started`, `Mic: engine configuration changed`, `Created
    /// aggregate device` and the first `Audio format:` line there.
    ///
