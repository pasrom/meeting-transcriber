...
/// comment). The live proof that a missing `--via` breaks the About tab lives
/// in `scripts/test_rpc.sh`.
final class UIPressTests: XCTestCase {
    ...
    func testViaClickIsIncludedInRequestPayload() throws {
        let command = try UIPress.parse(["someIdentifier", "--window", "settings", "--via", "click"])
        XCTAssertEqual(command.requestPayload, [
