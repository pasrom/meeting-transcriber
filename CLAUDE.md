...
- `echo.removed` on `GET /v1/jobs/<id>` is the only machine-readable evidence, and is three-state: absent (never attempted), `false` (ran, not confirmed), `true`. The middle one is the population a field soak counts.
