...
app/MeetingTranscriber/    # Swift macOS menu bar app (SPM)
  ...
  Sources/
    ...
    DebugRPCServer+UITree.swift # GET /ui/tree: in-process NSAccessibility tree walk → JSON (read-only, allowlisted windows, no TCC)
    DebugRPCServer+UIPress.swift # POST /ui/press: in-process accessibilityPerformPress() of a control by identifier (allowlisted windows, no TCC)
