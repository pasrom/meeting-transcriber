...
- **Optional Anthropic API key (Settings → Output, `AppSettings.claudeAPIKey`, Keychain-backed like `openAIAPIKey`):** lets the Claude CLI subprocess authenticate via `ANTHROPIC_API_KEY` when its own `claude login` OAuth session can't stay signed in. Never auto-discovered — `buildEnvironment` injects it only when the user has typed one into Settings and the base environment carries no key of its own — because silently preferring a found key would move a healthy subscription session onto metered billing, and the CLI doesn't fall back to OAuth on a bad key (it retries for ~3 minutes, then fails). Empty (the default) reproduces the previous CLI-only behavior exactly.
