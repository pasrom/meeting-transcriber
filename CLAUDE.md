...
- `InFlightRunRegistry` is a process-wide, in-memory guard against two `PipelineQueue` instances claiming the same job ID or audio path at once — `PipelineQueue.isProcessing` alone only covers one queue instance, and a replacement queue built while the previous one is still running knows nothing about it (issue #558).
...
- `OpenAIProtocolGenerator` supports any OpenAI-compatible HTTP API (Ollama, LM Studio, llama.cpp, etc.). It sends `max_tokens` first and retries once with `max_completion_tokens` when the endpoint's HTTP 400 explicitly names both parameters (GPT-5-family models reject the old name; local servers still expect it).
