...
- **Cancellation.** `EchoCanceller` wraps LocalVQE v1.4-AEC (Apache-2.0, prebuilt static library via `binaryTarget` from `pasrom/localvqe-xcframework`; the 2.8 MB weights are fetched once from the upstream Hugging Face repo at a pinned revision, SHA-256 verified, cached under `AppPaths.dataDir/models`). Echo-only by design: a joint denoise/dereverb model would move the spectrum the WeSpeaker embeddings depend on. Runs when the detector says affected and `AppSettings.echoCancellationEnabled` (default **on** — an affected recording is broken whether or not the user finds a switch; the setting turns processing off, it is not a gate in front of it).
- **It overwrites `workDir/mic_16k.wav` in place, and that is the design.** Transcription, `runDualTrackDiarization`, and the `<slug>_mic_16k.wav` sidecar that late re-diarization reads all take that one path, so cleaning it once fixes every consumer instead of leaving the first pass and later passes disagreeing. The recorder's raw `_mic.wav` is untouched.
- **The library's header is wrong about alignment**: it claims the output is sample-aligned, but the output lags by exactly one 256-sample hop (measured by cross-correlation, pinned in `EchoCancellerTests`). `EchoCanceller.realign` compensates; uncompensated, every mic segment would sit 16 ms late against the app track it is merged with.
- A cleaned recording reads as `EchoVerdict.clean`, so the embedding quarantine applies only to bleed that is still present (`detected && !cancelled`). Every cancellation failure is soft: the audio is left as recorded and the warning still stands.
