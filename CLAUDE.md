...
**Live captions (PoC):** "Show partial transcripts during recording" in Settings → Transcribe (`AppSettings.liveTranscriptionEnabled`, off by default; enabling downloads a ~0.6 GB model on first use behind a consent alert). Nested "Show caption overlay" (`AppSettings.liveCaptionsOverlayEnabled`, default on) hides `LiveCaptionsOverlay` without stopping the pipeline; `LiveCaptionsState` and `/state.liveCaptions` keep updating.
