...
- **Microphone-only recording (issue #633):** `RecordingSource` (`.appAndMic` / `.appOnly` / `.micOnly`) replaces the old `(appPID, noMic)` pair so "microphone, no app tap" — a meeting happening in the room rather than in an app — is representable without also making "no channels at all" representable. Started via the menu bar's "Record Microphone" item or `POST /v1/record`; the permission gate and channel-health monitors switch on `RecordingSource`/`CapturedChannels` rather than a PID so a deliberately-absent channel isn't misread as a dead one.
...
- Versioned automation API under `/v1` (carries a stability contract, kept off the debug `/action/*` surface): `POST /v1/transcribe` (blocking one-call: 200 terminal / 202 still-running / 400), `POST /v1/jobs` + `GET /v1/jobs/<id>` (enqueue + poll), `GET`/`POST /v1/jobs/<id>/naming` + `POST /v1/jobs/<id>/naming/skip` (speaker naming; 409 on wrong state, 404 unknown id), `GET`/`POST /v1/watch` (meeting-watching status/control: start/stop/toggle) and `GET`/`POST /v1/record` (microphone-only recording status/control, issue #633; 412 when nothing would be captured — no mic or the mic permission is denied/broken — 409 when another recording already owns the loop). The two POST enqueue routes honour an `Idempotency-Key` header. Finished-job readback survives the 60s queue reaping + an app restart via `TerminalJobStore`. Full reference: `docs/automation-api.md`. Routing in `DebugRPCServer+V1.swift`.
...
- `tools/mt-cli` is the matching CLI client (`mt-cli watch [status|start|stop|toggle]` and `mt-cli record [status|start|stop|toggle]` drive `/v1/watch` and `/v1/record`); `scripts/test_rpc.sh` is a live end-to-end smoketest. In-process integration tests live in `Tests/DebugRPCServerIntegrationTests.swift` (real sockets via OS-assigned port exposed through `DebugRPCServer.boundPort`).
